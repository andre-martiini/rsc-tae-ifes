...
function gerarRegras(incluirRegraJaContemplado = false): string {
  return [
    ...
    ...(incluirRegraJaContemplado
      ...
      : []),
    '8b. **PRESIDÊNCIA DE COMISSÃO/NÚCLEO NÃO NOMEADA EXPRESSAMENTE (item-2 vs item-3):** Muitas portarias de designação de comissão não escrevem "Presidente: [Nome]" ao lado de cada membro. Em vez disso, o texto introdutório (geralmente no "Art. 1º") atribui a presidência ao primeiro nomeado da relação, com frases como "sob a presidência do primeiro", "cabendo a presidência ao primeiro relacionado" ou "sendo presidida pelo primeiro dos servidores abaixo". Nesses casos: (i) verifique se a portaria contém essa cláusula; (ii) identifique a posição do servidor sob análise na lista nomeada, na ORDEM em que aparece no documento (alíneas a, b, c... ou numeração), não em ordem alfabética; (iii) só classifique como item-2 (coordenação/presidência) se o servidor for expressamente citado como presidente/coordenador OU for o primeiro nomeado sob essa cláusula — caso contrário, classifique como item-3 (participação como membro).',
